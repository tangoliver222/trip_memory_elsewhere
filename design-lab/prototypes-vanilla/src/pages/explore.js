...
function renderTimeView(slug) {
  return `<section class="time-explore">
    ...
    <div class="time-moments">
      ${scenes.map((moment, index) => `<button type="button" class="time-moment" data-action="navigate" data-route="${getAuthorityLink('scene', moment.id)}" style="--moment-order:${index}" data-particle-anchor="timeline-scene-${index + 1}" data-particle-kind="scene" data-particle-depth="${[-22, -13, -7, -17, -3][index]}">
...
function renderPlaceView() {
  return `<section class="place-explore">
    <div class="memory-map" aria-label="Bangkok 记忆地点图">
      ...
      ${places.map((place, index) => `<button class="map-place map-place--${index + 1}" type="button" data-action="navigate" data-route="${getAuthorityLink('place', place.id)}" data-particle-anchor="map-place-${index + 1}" data-particle-kind="place" data-particle-depth="${[-8, -14, -20, -11][index]}"><i></i><span>${escapeHtml(place.name)}</span><small>${escapeHtml(place.area)}</small></button>`).join('')}
    </div>
...
function renderConnectionView() {
  return `<section class="connection-explore">
    <div class="relation-space">
      ${connections.map((connection, index) => `<button class="relation-node relation-node--${index + 1} relation-node--${connection.status}" type="button" data-action="navigate" data-route="${getAuthorityLink('connection', connection.id)}" data-particle-anchor="relation-node-${index + 1}" data-particle-kind="connection" data-particle-depth="${[-18, -8, -13, -24, -5][index]}"><span>${{ same_visit: '同一次到访', repeated_place: '重复地点', temporal_and_textual_near: '时间与文字靠近', place_candidate: '地点候选' }[connection.type]}</span><small>${connection.evidence.length} 条来源说明</small></button>`).join('')}
  ...
  return {
    ...
    html: `<main class="capsule-page" data-page-id="world-capsule">
      ...
      <section class="capsule-chapter capsule-chapter--ari">
        ...
        <div class="capsule-spread capsule-reveal">${ari.slice(0, 3).map((item, index) => renderOriginalTile(item, { index, label: item.evidencePreview, anchorId: `capsule-ari-${index + 1}`, particleDepth: [-15, -8, -21][index] })).join('')}</div>
      ...
      <section class="capsule-chapter capsule-chapter--river">
        ...
        <div class="capsule-river-originals capsule-reveal">${river.map((item, index) => renderOriginalTile(item, { index, label: item.evidencePreview, anchorId: `capsule-river-${index + 1}`, particleDepth: [-18, -6, -24][index] })).join('')}</div>
        ${renderTimeBridge({ from: '17:42', to: '17:59', duration: '17 分钟' })}
      </section>
      <section class="capsule-chapter capsule-chapter--ending"><div class="capsule-ending__image capsule-reveal" data-particle-anchor="capsule-ending-original" data-particle-kind="fragment" data-particle-depth="-5"><img src="/assets/bangkok-photo-02-riverside.jpg" alt="Chao Phraya 河岸原件"></div><div class="capsule-ending__words capsule-reveal"><p>CHAPTER 03 · LEAVING</p><blockquote>“离开 Bangkok 以后，我最常想起的是河上的风。”</blockquote><span>这是你的文字；Elsewhere 没有替你补写感受。</span></div></section>
...
export function renderSceneDetail(sceneId) {
  ...
  return {
    ...
    html: `<main class="page authority-page moment-detail" data-page-id="world-scene-detail">
      <section class="moment-evidence" data-authority-evidence>${originals.map((item, index) => `<div class="moment-evidence__item"><span>${escapeHtml(item.capturedAt.slice(11,16))}</span>${renderOriginalTile(item, { interactive: true, index, label: item.evidencePreview, anchorId: `moment-original-${index + 1}`, particleDepth: [-15, -5, -22][index] })}</div>`).join('')}${originals.length > 1 ? renderTimeBridge({ from: originals[0].capturedAt.slice(11,16), to: originals[1].capturedAt.slice(11,16), duration: '17 分钟' }) : ''}</section>
...
export function renderPlaceDetail(placeId) {
  ...
  return {
    ...
    html: `<main class="page authority-page place-detail" data-page-id="world-place-detail">
      <div class="place-anchor" aria-hidden="true" data-particle-anchor="place-detail-entity" data-particle-kind="place" data-particle-depth="-8"><i></i><i></i><i></i><span>${escapeHtml(place.visitCount ?? '?')}</span></div>
...
export function renderConnectionDetail(connectionId) {
  ...
  return {
    ...
    html: `<main class="page authority-page connection-detail" data-page-id="world-connection-detail">
      <section class="connection-originals" data-authority-evidence>${originals.length ? originals.map((item, index) => renderEvidenceCard(item, { title: item.evidencePreview, detail: item.placeCandidate, anchorId: `connection-original-${index + 1}`, particleDepth: [-15, -5, -22][index] })).join('') : '<div class="missing-original">原件尚未加入当前演示包</div>'}</section>
      <div data-particle-anchor="connection-detail-thread" data-particle-kind="connection" data-particle-depth="-9">${renderRelationThread({ label: humanType, evidence: connection.evidence, uncertainty: gap, state: connection.status })}</div>
