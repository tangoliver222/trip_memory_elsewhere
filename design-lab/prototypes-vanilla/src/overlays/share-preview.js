...
export function renderSharePreview(payload = {}) {
  ...
  const text = payload.text || model.visibleContent.summary;
  return `<section class="share-preview spatial-overlay" role="dialog" aria-modal="true" aria-label="分享预览">
    ...
    <article class="share-preview__panel">
      ...
      <div class="share-preview__card"><div class="share-preview__particles" aria-hidden="true"><i></i><i></i><i></i></div><p>${escapeHtml(text)}</p><span>ELSEWHERE · PRIVATE MEMORY CUT</span></div>
      ...
      <button class="primary-action" type="button" data-action="confirm-share" data-title="${escapeHtml(title)}" data-text="${escapeHtml(text)}">保存分享图</button>
    </article>
  </section>`;
}
